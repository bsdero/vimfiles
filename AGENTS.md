...
- **All mappings added after the initial cleanup pass live together in a `=> Custom mappings` section**, near the end of the file (after `=> Session key maps`, before the `vim-quickui` menu definitions) — check there first. It holds: `<leader>rc` (the `^M`-strip mapping, relocated from `=> Misc`, renamed from `<Leader>m`), `<leader>m0`/`<leader>m1` (mouse off/on), `<leader>rs` (strip trailing whitespace), `<leader>rw` (toggle wrap — deliberately not `<leader>wr`, since `<leader>w` alone is the fast-save mapping), `<leader>rn` (coc rename), `<leader>cn`/`<leader>cp` (colorscheme cycling, see `g:my_colorschemes`), and `<leader>ev` (quick-edit `$MYVIMRC`, current window not a new tab). The session mappings (`<leader>So`/`Ss`/`Sc`/`Sd`/`Sl`) live in their own `=> Session key maps` section just above this one, not inside it.
- **`coc.nvim`** provides autocompletion/LSP (see `=> Coc.nvim configuration`, just below `=> ALE configuration`): `Tab`/`Shift-Tab` cycle the popup, `Enter` confirms, `K` shows hover docs, `gd`/`gy`/`gi`/`gr` go to definition/type/implementation/references, `<leader>rn` renames. `g:coc_global_extensions` covers Python (`coc-pyright`), C/C++ (`coc-clangd`), JSON (`coc-json`), shell (`coc-sh`), and Rust (`coc-rust-analyzer`) — coc auto-installs these on first launch. Perl has no marketplace extension, so it's wired up as a generic `languageserver` entry in `g:coc_user_config` pointing at `Perl::LanguageServer` (install separately via `cpanm Perl::LanguageServer`) — **unverified**, since installing CPAN modules wasn't possible in the sandbox this was authored in; check `:CocInfo` on a `.pl` file before relying on it. `diagnostic.enable` is set to `v:false` in `g:coc_user_config` so coc doesn't draw its own gutter signs alongside ALE's — ALE remains the sole linting/diagnostics source. **Not verified interactively:** whether coc's blanket `<CR>` override breaks auto-pairs' own bracket-expand-on-Enter behavior.
...
Autocompletion (`coc.nvim`) has been added — see the `coc.nvim` bullet above. Nothing else is currently tracked as pending; check README.md's "Pending tasks" section for anything newer.
